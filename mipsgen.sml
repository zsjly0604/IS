...
fun codegen (stm:T.stm) : A.instr list = 
  let val ilist = ref (nil:A.instr list)
      ...
      fun munchStm (T.SEQ(a, b)) = (munchStm a; munchStm b)
        ...
        (*sw*)
        | munchStm (T.MOVE(T.MEM(T.BINOP(T.PLUS, e1, T.CONST i)), e2)) = emit(A.OPER{assem = "sw 's0, "^ Int.toString i ^ "('s1)\n", 
                                                                                                        src = [munchExp e2, munchExp e1], dst = [], jump = NONE})
        | munchStm (T.MOVE(T.MEM(T.BINOP(T.PLUS, T.CONST i, e1)), e2)) =  emit(A.OPER{assem = "sw 's0, "^ Int.toString i ^ "('s1)\n", 
                                                                                                         src = [munchExp e2, munchExp e1], dst = [], jump = NONE})
	| munchStm (T.MOVE(T.MEM(T.BINOP(T.MINUS, e1, T.CONST i)), e2)) = emit(A.OPER{assem = "sw 's0, -" ^ Int.toString i ^ "('s1)\n",
                             src = [munchExp e2, munchExp e1], dst = [], jump = NONE})
	| munchStm (T.MOVE(T.MEM(T.BINOP(T.MINUS, T.CONST i, e1)), e2)) = emit(A.OPER{assem = "sw 's0, -" ^ Int.toString i ^ "('s1)\n",
                             src = [munchExp e2, munchExp e1], dst = [], jump = NONE})
        | munchStm (T.MOVE(T.MEM(T.CONST i), e)) = emit(A.OPER{assem = "sw 's0, "  ^ Int.toString i ^ "($ZERO)\n", 
 src = [munchExp e], dst = [], jump = NONE})
        | munchStm (T.MOVE(T.MEM(e1), e2)) = emit(A.OPER{assem = "sw 's0, 0('s1)\n", 
                                                                   src = [munchExp e2, munchExp e1], dst = [], jump = NONE})
        (*li*)
	| munchStm (T.MOVE(T.TEMP t, T.CONST i)) = emit(A.OPER{assem = "li 'd0, " ^ Int.toString i ^ "\n",
                                                                   src =[], dst = [t], jump = NONE})
        (*lw*)
	| munchStm (T.MOVE(T.TEMP t, T.BINOP(T.PLUS, e1, T.CONST i))) = emit(A.OPER{assem = "lw 'd0, " ^ Int.toString i ^ "('s0)\n",
            src =[], dst = [t], jump = NONE})
	| munchStm (T.MOVE(T.TEMP t, T.BINOP(T.PLUS, T.CONST i, e1))) = emit(A.OPER{assem = "lw 'd0, " ^ Int.toString i ^ "('s0)\n",
            src =[], dst = [t], jump = NONE})
	| munchStm (T.MOVE(T.TEMP t, T.BINOP(T.MINUS, e1, T.CONST i))) = emit(A.OPER{assem = "lw 'd0, " ^ Int.toString (~i) ^ "('s0)\n",
            src =[], dst = [t], jump = NONE})
	| munchStm (T.MOVE(T.TEMP t, T.BINOP(T.MINUS, T.CONST i, e1))) = emit(A.OPER{assem = "lw 'd0, " ^ Int.toString (~i) ^ "('s0)\n",
            src =[], dst = [t], jump = NONE})
        (*move*)
        | munchStm (T.MOVE(T.TEMP t, e)) = emit(A.OPER{assem = "move 'd0, 0('s0)\n", 
	...
	| munchStm (T.CJUMP(T.LT, e1, e2, l1, l2)) = emit(A.OPER{assem = "blt 's0, 's1, 'j0\n b 'j1\n",
                                                                        src = [munchExp e1, munchExp e2], dst = [], jump = SOME([l1, l2])})                     | munchStm (T.CJUMP(T.EQ, e1, e2, l1, l2)) = emit(A.OPER{assem = "beq 's0, 's1, 'j0\n b 'j1\n",
                                                                        src = [munchExp e1, munchExp e2], dst = [], jump = SOME([l1, l2])})
	...
	| munchStm (T.EXP e) = (munchExp e;())
      ...
      and munchExp (T.MEM(T.CONST i)) = result(fn r => emit(A.OPER{assem = "lw 'd0, " ^ Int.toString i ^"($ZERO)\n",                                                                                                                src = [],dst = [r],jump = NONE}))
	| munchExp (T.MEM(T.BINOP(T.PLUS, e, T.CONST i))) = result(fn r => emit(A.OPER{assem = "lw 'd0, " ^ Int.toString i ^ "('s0)\n",
             src = [munchExp e],dst = [r], jump = NONE}))
        | munchExp (T.MEM(T.BINOP(T.PLUS, T.CONST i, e))) = result(fn r => emit(A.OPER{assem = "lw 'd0, " ^ Int.toString i ^ "('s0)\n",
           src = [munchExp e],dst = [r],jump = NONE}))
	| munchExp (T.MEM(T.BINOP(T.MINUS, e, T.CONST i))) = result(fn r => emit(A.OPER{assem = "lw 'd0, " ^ Int.toString (~i) ^ "('s0)\n",
             src = [munchExp e],dst = [r], jump = NONE}))
        | munchExp (T.MEM(T.BINOP(T.MINUS, T.CONST i, e))) = result(fn r => emit(A.OPER{assem = "lw 'd0, " ^ Int.toString (~i) ^ "('s0)\n",
           src = [munchExp e],dst = [r],jump = NONE}))
        (*add/addiu*)
        | munchExp (T.BINOP(T.PLUS, e, T.CONST i)) = result(fn r => emit(A.OPER{assem = "addu 'd0, 's0" ^ Int.toString i ^ "\n",
    src = [munchExp e],dst = [r],jump = NONE}))
        | munchExp (T.BINOP(T.PLUS, T.CONST i, e)) = result(fn r => emit(A.OPER{assem = "addu 'd0, 's0" ^ Int.toString i ^ "\n",
    src = [munchExp e],dst = [r],jump = NONE}))                            
        | munchExp (T.BINOP(T.PLUS, e1, e2)) =  result(fn r => emit(A.OPER{assem = "add 'd0, 's0, 's1\n",
                                                                           src = [munchExp e1, munchExp e2],dst = [r],jump = NONE}))
        (*sub*)
	| munchExp (T.BINOP(T.MINUS, e, T.CONST i)) = result(fn r => emit(A.OPER{assem = "subu 'd0, 's0, " ^ Int.toString i ^ "\n",
     src = [munchExp e], dst = [r], jump = NONE})) 
        | munchExp (T.BINOP(T.MINUS, e1, e2)) =  result(fn r => emit(A.OPER{assem = "sub 'd0, 's0, 's1\n",
src = [munchExp e1, munchExp e2],dst = [r],jump = NONE}))
        (*div*)
        | munchExp (T.BINOP(T.DIV, e1, e2)) =  result(fn r => emit(A.OPER{assem = "add 'd0, 's0, 's1\n",
                                                                           src = [munchExp e1, munchExp e2],dst = [r],jump = NONE}))
        (*mul*)
        | munchExp (T.BINOP(T.MUL, e1, e2)) =  result(fn r => emit(A.OPER{assem = "mul 'd0, 's0, 's1\n",
                                                                           src = [munchExp e1, munchExp e2],dst = [r],jump = NONE}))
        (*and*)
	| munchExp (T.BINOP(T.AND, e, T.CONST i)) = result(fn r => emit(A.OPER{assem = "andu 'd0, 's0, " ^ Int.toString i ^ "\n",
   src = [munchExp e], dst = [r], jump = NONE}))  
	| munchExp (T.BINOP(T.AND, T.CONST i, e)) = result(fn r => emit(A.OPER{assem = "andu 'd0, 's0, " ^ Int.toString i ^ "\n",
   src = [munchExp e], dst = [r], jump = NONE}))
	| munchExp (T.BINOP(T.AND, e1, e2)) = result(fn r => emit(A.OPER{assem = "and 'd0, 's0, 's1\n",
   src = [munchExp e1, munchExp e2], dst = [r], jump = NONE}))  
        (*or*)
	| munchExp (T.BINOP(T.OR, e, T.CONST i)) = result(fn r => emit(A.OPER{assem = "oru 'd0, 's0, " ^ Int.toString i ^ "\n",
   src = [munchExp e], dst = [r], jump = NONE}))  
	| munchExp (T.BINOP(T.OR, T.CONST i, e)) = result(fn r => emit(A.OPER{assem = "oru 'd0, 's0, " ^ Int.toString i ^ "\n",
   src = [munchExp e], dst = [r], jump = NONE}))
	| munchExp (T.BINOP(T.OR, e1, e2)) = result(fn r => emit(A.OPER{assem = "or 'd0, 's0, 's1\n",
   src = [munchExp e1, munchExp e2], dst = [r], jump = NONE}))    
        (*li*)
	| munchExp (T.CONST i) = result(fn r => emit(A.OPER{assem = "li 'd0, "^ Int.toString i ^ "\n",
                                                            src=[], dst = [r], jump = NONE}))
        (*TEMP*)
        | munchExp (T.TEMP t) = t 
        (*la*)
